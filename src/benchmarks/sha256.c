...
#define RORc(x, y)                                                             \
  (((((ulong32)(x)&0xFFFFFFFFUL) >> (ulong32)((y)&31)) |                       \
    ((ulong32)(x) << (ulong32)((32 - ((y)&31)) & 31))) &                       \
   0xFFFFFFFFUL)

/* a simple macro for making hash "process" functions */
#define HASH_PROCESS(func_name, compress_name, state_var, block_size)          \
  static int func_name(hash_state *md, const unsigned char *in,                \
                       unsigned long inlen) {                                  \
    unsigned long n;                                                           \
    int err;                                                                   \
    LTC_ARGCHK(md != NULL);                                                    \
...
*/
static int compare_testvector(const void *is, const unsigned long is_len,
...
static int sha256_test(void) {
  ...
  for (i = 0; i < (int)(sizeof(tests) / sizeof(tests[0])); i++) {
    sha256_init(&md);
    sha256_process(&md, (const unsigned char *)tests[i].msg,
...
static void *SHA256_argument_init(void *arg_) {
  assert(arg_ == NULL);

  SHA256_t *arg = (SHA256_t *)malloc(sizeof(SHA256_t));
  if (arg == NULL) {
    exit(EXIT_FAILURE);
  }

  arg->length = size;
  arg->buf = (unsigned char *)malloc(arg->length);
  if (arg->buf == NULL) {
    exit(EXIT_FAILURE);
  }

  memset(arg->buf, 0, arg->length);
  sha256_init(&arg->md);

  return arg;
}

static void SHA256_argument_destroy(void *arg_) {
...
static void *SHA256_call(void *arg_) {
  SHA256_t *arg = (SHA256_t *)arg_;
